...
// in LISTEN, send ACK
static void ack_listen_test(const TCPConfig &cfg, const WrappingInt32 seqno,
                            const WrappingInt32 ackno, const int lineno) {
  try {
    ...
    throw runtime_error(string(e.what()) +
                        " (ack_listen_test called from line " +
                        to_string(lineno) + ")");
  }
}
...
static void ack_rst_syn_sent_test(const TCPConfig &cfg,
                                  ...
                                  const bool send_rst, const int lineno) {
  try {
    ...
    throw runtime_error(string(e.what()) +
                        " (ack_rst_syn_sent_test called from line " +
                        to_string(lineno) + ")");
  }
}
  ...
  try {
    ...
    // test #1: in ESTABLISHED, send unacceptable segments and ACKs
    {
      ...
      TCPTestHarness test_1 =
          TCPTestHarness::in_established(cfg, base_seq - 1, base_seq - 1);
      ...
      test_1.execute(ExpectUnassembledBytes{0},
      ...
      test_1.execute(ExpectUnassembledBytes{0},
                     "test 1 failed: seg queued on late seqno");
      test_1.execute(ExpectOneSegment{}.with_ack(true).with_ackno(base_seq),
                     "test 1 failed: bad ACK on late seqno");

      // packet next byte in the window - ack should advance and data should be
      // readable
      test_1.send_byte(base_seq, base_seq, 1);

      test_1.execute(ExpectData{},
                     "test 1 failed: pkt not processed on next seqno");
    ...
    // test #2: in LISTEN, send RSTs
    {
      ...
      test_2.execute(ExpectNoSegment{},
                     "test 2 failed: RST was not ignored in LISTEN");
    }
    ...
    ack_listen_test(cfg, base_seq + cfg.recv_capacity, base_seq, __LINE__);
    ack_listen_test(cfg, base_seq + cfg.recv_capacity,
                    base_seq + cfg.recv_capacity, __LINE__);
    ...
    // test 4: ACK and RST in SYN_SENT
    {
      ...
      test_4.execute(ExpectNoSegment{},
                     "test 4 failed: bytes were not ignored in SYN_SENT");
    ...
    ack_rst_syn_sent_test(cfg, base_seq, base_seq, base_seq + 2, false,
                          __LINE__);
    ack_rst_syn_sent_test(cfg, base_seq, base_seq, base_seq, true, __LINE__);
    ack_rst_syn_sent_test(cfg, base_seq, base_seq, base_seq + 2, true,
                          __LINE__);
  } catch (const exception &e) {
    cerr << e.what() << endl;
    return EXIT_FAILURE;
