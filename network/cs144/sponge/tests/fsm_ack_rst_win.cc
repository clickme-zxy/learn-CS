...
int main() {
  try {
    ...
    // test #1: in ESTABLISHED, send unacceptable segments and ACKs
    {
      ...
      test_1.execute(ExpectOneSegment{}.with_ackno(base_seq),
                     "test 1 failed: bad ACK after future ACK");
      ...
      test_1.execute(ExpectOneSegment{}.with_ackno(base_seq),
                     "test 1 failed: no ack on late seqno");

      // segment in the window but late---should get an ACK and seg should be
      // queued
      test_1.send_byte(base_seq + cfg.recv_capacity - 1, base_seq, 1);

      test_1.execute(ExpectUnassembledBytes{1},
                     "seg not queued on end-of-window seqno");

      test_1.execute(ExpectOneSegment{}.with_ackno(base_seq),
                     "test 1 failed: no ack on end-of-window seqno");
      test_1.execute(ExpectNoData{},
                     "test 1 failed: no ack on end-of-window seqno");

      // segment next byte in the window - ack should advance and data should be
      // readable
      test_1.send_byte(base_seq, base_seq, 1);

      test_1.execute(ExpectUnassembledBytes{1},
                     "seg not processed on next seqno");
      test_1.execute(ExpectOneSegment{}.with_ackno(base_seq + 1),
                     "test 1 failed: no ack on next seqno");
