...
int main() {
  try {
    ...
    // NOTE: the timeouts in this test are carefully adjusted to work whether
    // the tcp_state_machine sends immediately upon write() or only after the
    // next tick(). If you edit this test, make sure that it passes both ways
    // (i.e., with and without calling _try_send() in TCPConnection::write())
    ...
    // single segment re-transmit
    {
      ...
      TCPTestHarness test_1 =
          TCPTestHarness::in_established(cfg, tx_ackno - 1, tx_ackno - 1);
      ...
      for (unsigned i = 2; i < TCPConfig::MAX_RETX_ATTEMPTS; ++i) {
        test_1.execute(Tick((cfg.rt_timeout << i) -
                            i)); // exponentially increasing delay length
        test_1.execute(ExpectNoSegment{},
                       "test 1 failed: re-tx too fast after timeout");
      ...
      test_1.execute(
          Tick(1 + (cfg.rt_timeout << TCPConfig::MAX_RETX_ATTEMPTS)));
      test_1.execute(ExpectState{State::RESET});
      test_1.execute(
          ExpectOneSegment{}.with_rst(true).with_ack(false).with_seqno(
              tx_ackno),
