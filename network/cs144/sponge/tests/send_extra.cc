...
int main() {
  try {
...

    {
      ...
      printf("33\n");
      test.execute(
          ExpectSegment{}.with_payload_size(3).with_data("abc").with_seqno(isn +
                                                                           1));
      printf("33\n");
...

    {
      ...
      test.execute(WriteBytes("def"));
      test.execute(
          ExpectSegment{}.with_payload_size(3).with_data("def").with_seqno(isn +
                                                                           4));
...

    {
      ...
      test.execute(
          ExpectSegment{}.with_payload_size(3).with_data("ghi").with_seqno(isn +
                                                                           7));
      test.execute(AckReceived{WrappingInt32{isn + 1}}.with_win(1000));
      test.execute(Tick{6});
      test.execute(
...

    {
      ...
      test.execute(Close{});
      test.execute(
          ExpectSegment{}.with_payload_size(0).with_seqno(isn + 4).with_fin(
              true));
      test.execute(Tick{rto - 1});
      test.execute(ExpectNoSegment{});
...

    {
      ...
      test.execute(
          ExpectSegment{}.with_payload_size(7).with_data("abcdefg").with_seqno(
              isn + 1));
