...
TestFD::TestFD()
    : TestFD([&]() {
        ...
        SystemCall("socketpair",
                   socketpair(AF_UNIX, SOCK_SEQPACKET, 0, &fds[0]));
...
bool TestFD::TestRFD::can_read() const {
  uint8_t tmp;
  return SystemCall("recv", ::recv(fd_num(), &tmp, 1, MSG_PEEK | MSG_DONTWAIT),
                    EAGAIN) >= 0;
}
...
string TestFD::TestRFD::read() {
  ...
  ssize_t ret_read =
      SystemCall("recv", ::recv(fd_num(), ret.data(), ret.size(), MSG_TRUNC));
...
//! \param[in] ackno is the optional acknowledgment number of the segment; if no
//! value, ACK flag is not set
void TCPTestHarness::send_fin(const WrappingInt32 seqno,
...
//! \param[in] swin is the optional window size for the segment; if no value,
//! uses default value (137 bytes)
void TCPTestHarness::send_ack(const WrappingInt32 seqno,
  ...
  execute(
      SendSegment{}.with_ack(true).with_ackno(ackno).with_seqno(seqno).with_win(
          win));
}
...
//! \param[in] ackno is the optional acknowledgment number of the segment; if no
//! value, ACK flag is not set
void TCPTestHarness::send_rst(const WrappingInt32 seqno,
                              const optional<WrappingInt32> ackno) {
  SendSegment step{};
  if (ackno.has_value()) {
    step.with_ack(true).with_ackno(ackno.value());
...
//! \param[in] ackno is the optional acknowledgment number of the segment; if no
//! value, ACK flag is not set
void TCPTestHarness::send_syn(const WrappingInt32 seqno,
                              const optional<WrappingInt32> ackno) {
  SendSegment step{};
  if (ackno.has_value()) {
    step.with_ack(true).with_ackno(ackno.value());
...
//! \param[in] seqno is the sequence number of the segment
//! \param[in] ackno is the optional acknowledgment number of the segment; if no
//! value, ACK flag is not set \param[in] val is the value of the one-byte
//! payload
...
void TCPTestHarness::send_data(const WrappingInt32 seqno,
                               const WrappingInt32 ackno, VecIterT begin,
                               VecIterT end) {
  SendSegment step{};
  execute(SendSegment{}
              .with_ack(true)
...
TCPSegment TCPTestHarness::expect_seg(const ExpectSegment &expectation,
                                      std::string note) {
  try {
...
//!            seqno for the SYN.
TCPTestHarness TCPTestHarness::in_syn_sent(const TCPConfig &cfg,
...
//! ACK'd \param[in] tx_isn is the ISN of the FSM's outbound sequence. i.e. the
//!            seqno for the SYN.
...
TCPTestHarness TCPTestHarness::in_close_wait(const TCPConfig &cfg,
  ...
  h.execute(
      ExpectOneSegment{}.with_no_flags().with_ack(true).with_ackno(rx_isn + 2));
  return h;
}

//! \brief Create an FSM in LAST_ACK
//! \details SYNs have been traded, then the machine received and ACK'd FIN, and
//! then it sent its own FIN. \param[in] tx_isn is the ISN of the FSM's outbound
//! sequence. i.e. the
//!            seqno for the SYN.
//! \param[in] rx_isn is the ISN of the FSM's inbound sequence. i.e. the
//!            seqno for the SYN.
