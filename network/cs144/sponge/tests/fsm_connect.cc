...
int main() {
  try {
    ...
    // test #1: START -> SYN_SENT -> ACK (ignored) -> SYN -> SYN_RECV
    {
      ...
      TCPSegment seg1 = test_1.expect_seg(
          ExpectOneSegment{}.with_syn(true).with_ack(false),
      ...
      test_1.execute(
          ExpectOneSegment{}.with_ack(true).with_syn(false).with_ackno(isn +
                                                                       1));
    ...
    // test #2: START -> SYN_SENT -> SYN -> ACK -> ESTABLISHED
    {
      ...
      TCPSegment seg = test_2.expect_seg(
          ExpectOneSegment{}.with_syn(true).with_ack(false),
      ...
      test_2.expect_seg(
          ExpectOneSegment{}.with_syn(false).with_ack(true).with_ackno(isn + 1),
      ...
      test_2.execute(ExpectNoSegment{},
                     "test 2 failed: got spurious ACK after ACKing SYN");
    ...
    // test #3: START -> SYN_SENT -> SYN/ACK -> ESTABLISHED
    {
      ...
      TCPSegment seg = test_3.expect_seg(
          ExpectOneSegment{}.with_syn(true).with_ack(false),
      ...
      test_3.execute(
          ExpectOneSegment{}.with_ack(true).with_ackno(isn + 1).with_syn(false),
