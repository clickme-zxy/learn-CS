...
//! \brief A 32-bit integer, expressed relative to an arbitrary initial sequence
//! number (ISN) \note This is used to express TCP sequence numbers (seqno) and
//! acknowledgment numbers (ackno)
class WrappingInt32 {
private:
...
//! Transform a 64-bit absolute sequence number (zero-indexed) into a 32-bit
//! relative sequence number \param n the absolute sequence number \param isn
//! the initial sequence number \returns the relative sequence number
WrappingInt32 wrap(uint64_t n, WrappingInt32 isn);

//! Transform a 32-bit relative sequence number into a 64-bit absolute sequence
//! number (zero-indexed) \param n The relative sequence number \param isn The
//! initial sequence number \param checkpoint A recent absolute sequence number
