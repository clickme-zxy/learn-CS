...
//! \param[in] errno_mask is any errno value that is acceptable, e.g., `EAGAIN`
//! when reading a non-blocking fd \details This function works for any syscall
//! that returns less than 0 on error and sets errno:
//!
...
//! ~~~
//!
//! If you don't have permission to open the file, SystemCall will throw a
//! std::runtime_error. If you don't want to throw in that case, you can pass
//! `EACCESS` in `errno_mask`:
//!
//! ~~~{.cc}
...
//! }
//! ~~~
int SystemCall(const char *attempt, const int return_value,
...
//! \param[in] return_value is the return value of the syscall
//! \param[in] errno_mask is any errno value that is acceptable, e.g., `EAGAIN`
//! when reading a non-blocking fd \details see the other SystemCall()
//! documentation for more details
int SystemCall(const string &attempt, const int return_value,
               const int errno_mask) {
...
//!       for rationale
//! \details This class can be used to either check or compute an Internet
//! checksum (e.g., for an IP datagram header or a TCP segment).
//!
//! The Internet checksum is defined such that evaluating inet_cksum() on a TCP
//! segment (IP datagram, etc) containing a correct checksum header will return
//! zero. In other words, if you read a correct TCP segment off the wire and
//! pass it untouched to inet_cksum(), the return value will be 0.
//!
//! Meanwhile, to compute the checksum for an outgoing TCP segment (IP datagram,
//! etc.), you must first set the checksum header to zero, then call
//! inet_cksum(), and finally set the checksum header to the return value.
//!
//! For more information, see the [Wikipedia
//! page](https://en.wikipedia.org/wiki/IPv4_header_checksum) on the Internet
//! checksum, and consult the [IP](\ref rfc::rfc791) and [TCP](\ref rfc::rfc793)
//! RFCs.
