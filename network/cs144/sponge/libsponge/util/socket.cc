...
//! SystemCall()) \param[in] function is a pointer to the function \returns the
//! requested Address
...
void UDPSocket::recv(received_datagram &datagram, const size_t mtu) {
  ...
  const ssize_t recv_len =
      SystemCall("recvfrom", ::recvfrom(fd_num(), datagram.payload.data(),
                                        datagram.payload.size(), MSG_TRUNC,
                                        datagram_source_address, &fromlen));
