...
private:
  ...
  using InterestT = std::function<bool(void)>; //!< `true` return indicates
                                               //!< Rule::fd should be polled.

  //! \brief Specifies a condition and callback that an EventLoop should handle.
  //! \details Created by calling EventLoop::add_rule() or
  //! EventLoop::add_cancelable_rule().
  class Rule {
  public:
    ...
    InterestT interest;  //!< A callback that returns `true` whenever fd should
                         //!< be polled.
...
public:
  ...
  //! Add a rule whose callback will be called when `fd` is ready in the
  //! specified Direction.
  void add_rule(const FileDescriptor &fd, const Direction direction,
...
//! \class EventLoop
//!
//! An EventLoop holds a std::list of Rule objects. Each time
//! EventLoop::wait_next_event is executed, the EventLoop uses the Rule objects
//! to construct a call to [poll(2)](\ref man2::poll).
//!
//! When a Rule is installed using EventLoop::add_rule, it will be polled for
//! the specified Rule::direction whenver the Rule::interest callback returns
//! `true`, until Rule::fd is no longer readable (for Rule::direction ==
//! Direction::In) or writable (for Rule::direction == Direction::Out). Once
//! this occurs, the Rule is canceled, i.e., the EventLoop deletes it.
//!
//! A Rule installed using EventLoop::add_cancelable_rule will be polled and
//! canceled under the same conditions, with the additional condition that if
//! Rule::callback returns `true`, the Rule will be canceled.
